...
// Handle bookintance borrow on GET
exports.bookinstance_borrow = function(req, res, next){
  BookInstance.findById(req.params.id).populate('book').exec((err, bookinstance) => {
    if (bookinstance.status != 'Available'){
      res.redirect(`/catalog/book/${bookinstance.book._id.toString()}`);
    }
    else {
      bookinstance.status = 'Loaned';
      bookinstance.borrower = req.user._id;
      bookinstance.due_back = Date.now() + 7 * 24 * 60 * 60 * 1000;

      BookInstance.findByIdAndUpdate(req.params.id, bookinstance).exec((err, theBookinstance) => {
        if (err) return next(err);
...
// Handle bookinstance reture on GET
exports.bookinstance_return = function(req, res, next){
  // console.log("trying to return ", req.params.id);
  BookInstance.findById(req.params.id).exec((err, bookinstance) => {
    // console.log("inside first find by ID err and result", err, bookinstance);
    if (bookinstance.status != 'Loaned' || bookinstance.borrower != req.user._id.toString()){
      res.redirect(`/catalog/book/${bookinstance.book._id.toString()}`);
    }
    else {
      ...
      BookInstance.findByIdAndUpdate(req.params.id, bookinstance).exec((err, theBookinstance) => {
        // console.log("err ", err);
        if (err) return next(err);
        console.log("theBookinstance ", theBookinstance);
        res.redirect(theBookinstance.url);
      });
    }
  });
